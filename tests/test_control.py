...
from pandablocks.control import BlockCompleter
...
def test_complete_one_block(dummy_server_with_blocks, blocking_client):
    completer = BlockCompleter(blocking_client)
    ...
    completer("LUT5.", 0)
    assert completer.matches == ["LUT5.INPA", "LUT5.TYPEA"]
    assert completer("LUT5.INPA?", 0) is None


def test_complete_stars(dummy_server_with_blocks, blocking_client):
    completer = BlockCompleter(blocking_client)
    ...
    assert completer("*DESC.LUT.", 0) == "*DESC.LUT.INPA"
    assert completer.matches == ["*DESC.LUT.INPA", "*DESC.LUT.TYPEA"]
